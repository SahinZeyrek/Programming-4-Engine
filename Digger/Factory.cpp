...
#include <iostream>
namespace dae::factory
{
	namespace 
	{
		// INTERNAL LINKAGE -> only accessible from this file
		// static non member method -> internal linkage
		static void InitializeLevelOne(Scene& scene)
		{
#if _DEBUG
...
#pragma endregion MOVE COMMANDS
		...
		static void InitializeStartScreen(Scene& scene)
		{
...
#pragma region Misc
			...
			RenderTextureComponent* rt_Background = new RenderTextureComponent(pBackground, nullptr);
			pBackground->AddComponent(rt_Background);
			pBackground->GetComponent<RenderTextureComponent>()->SetTexture("background.tga");
			pBackground->SetPosition(0, 40);
			scene->Add(pBackground);
		}
	...
	{
		if (scene.GetName() == "LevelOne") InitializeLevelOne(scene);
		if (scene.GetName() == "LevelTwo") InitializeStartScreen(scene);
	}
